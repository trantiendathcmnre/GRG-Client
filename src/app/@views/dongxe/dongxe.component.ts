...
export class DongxeComponent implements OnInit {
  ...
  dataHangXe:any = [ ];
  vehicles_list = 'Danh sách dòng xe';
  home = 'Trang chủ';
  categories = 'Quản lý danh mục';
  automakers = 'Sắp xếp theo hãng xe';
  all_of_automakers = 'Tất cả hãng xe';
  search = 'Tìm kiếm';
  new_vehicle = 'Thêm mới dòng xe';
  name_of_vehicle = 'Tên dòng xe';
  description = 'Mô tả';
  automaker = 'Hãng xe';
  action = 'Thao tác';
  please_select = 'Chọn hãng xe';
  cancel = 'Thoát';
  save = 'Lưu';
  ...
  ngOnInit() {
    this.title.setTitle('Danh Muc Dong Xe');
  ...
  private bindTableEvents()
  {
    ...
    $('a[data-group=grpDelete]').off('click').click(function(){
      ...
      $.confirm({
        title: 'Xóa dòng xe',
        content: 'Bạn có chắc muốn xóa dòng xe?',
        type: 'red',
        typeAnimated: true,
        buttons: {
            close:{
            ...
            tryAgain: {
                text: 'Xóa',
                btnClass: 'btn-success btn-flat bg green',
                action: function(){
